...
slow_start = 300 # lag for starting bcscore and hegemony code
...
if 'all' in analysis_type or 'data' in analysis_type:
    # Fetch BGP data
    for collector in all_collectors: 
        start_ribs = start_time.shift(hours=-1).strftime(DATE_FMT)
        end_ribs = start_time.shift(hours=1).strftime(DATE_FMT)
        print('# BGP ribs ', collector, start_str, end_str)
        childs.append(Popen(['python3', '../../../produce_bgpdata.py', '-c', collector, '-s', start_ribs, '-e', end_ribs, '-t', 'ribs']))
...
if 'all' in analysis_type or 'data' in analysis_type:
    # Fetch BGP data
    for collector in all_collectors: 
        print('# BGP updates ', collector, start_str, end_str)
        childs.append(Popen(['python3', '../../../produce_bgpdata.py', '-c', collector, '-s', start_str, '-e', end_str, '-t', 'updates']))
